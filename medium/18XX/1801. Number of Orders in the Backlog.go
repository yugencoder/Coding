...
func getNumberOfBacklogOrders(orders [][]int) int {
	...
	for _, o := range orders {
		if o[2] == 0 {
			...
			for ; ; {
				index, order := findOrdersGreaterThanEq(o, buyOrders)
				if index == -1 {
				...
				o[1] = o[1] - fulfilled
				if fulfilled == order[1] {
					buyOrders = append(buyOrders[:index], buyOrders[index+1:]...)
				} else {
					buyOrders[index][1] = order[1] - fulfilled
				}
				if o[1] == 0 {
					break
				}
...
func findOrdersGreaterThanEq(o []int, orders [][]int) (int, []int) {
